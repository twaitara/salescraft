    ...
    <div class="card pad">
      ...
      <?php foreach ($categories as $c): $p = $c['score'] / 20; ?>
        <div style="margin:10px 0">
          ...
          <div style="height:8px;background:#eef1f5;border-radius:99px;overflow:hidden">
    ...
    <?php foreach ($catNames as $si => $cat): ?>
      <div>
        ...
        <?php foreach ($titles[$cat] as $qi => $qt): $v = (int) ($answers["$si-$qi"] ?? 0); ?>
          <div style="display:flex;justify-content:space-between;align-items:center;font-size:13px;padding:5px 0;color:#41505f">
            <span><?= sc_e($qt) ?></span>
...
(function(){
  ...
  let rings='';[0.25,0.5,0.75,1].forEach(f=>{rings+=`<polygon points="${cats.map((_,i)=>pt(i,R*f).join(',')).join(' ')}" fill="none" stroke="#e7eaef"/>`;});
  let axes='',labels='';
  cats.forEach((c,i)=>{const[x,y]=pt(i,R);axes+=`<line x1="${cx}" y1="${cy}" x2="${x}" y2="${y}" stroke="#e7eaef"/>`;
    const[lx,ly]=pt(i,R+22);const anc=Math.abs(lx-cx)<8?'middle':(lx>cx?'start':'end');
    labels+=`<text x="${lx}" y="${ly}" font-size="10.5" fill="#7c8b99" text-anchor="${anc}" dominant-baseline="middle" font-weight="600">${c.name}</text>`;});
